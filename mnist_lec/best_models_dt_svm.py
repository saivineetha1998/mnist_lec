...
output_folder = "svm_dt_models"
# os.mkdir(output_folder)
fname = 'bst_nodel_svm.sav'
pickle.dump(clf, open(os.path.join(output_folder, fname), 'wb'))


# Decision Tree classifier
clf_dt = tree.DecisionTreeClassifier(max_depth = depth_tree)
clf_dt.fit(X_train, y_train)
val_predicted_dt = clf_dt.predict(X_val)
fname2 = 'bst_model_dt.sav'
pickle.dump(clf_dt, open(os.path.join(output_folder, fname2), 'wb'))
tets_predicted = clf.predict(X_test)
print(tets_predicted)
tets_predicted_dt = clf_dt.predict(X_test)
print(tets_predicted_dt)